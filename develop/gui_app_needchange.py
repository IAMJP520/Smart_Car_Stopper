...
class HyundaiStyleUI(QWidget):
    ...
    def launch_parking_ui(self):
        ...
        try:
            ...
            print(f"\n✅ 전송 성공! 다음 3가지 UI를 실행합니다:")
            
            # 1. main_controller.py 먼저 실행 (ZeroMQ 시작)
            main_controller_path = os.path.join(current_dir, 'main_controller.py')
            print(f"   1. main_controller.py 실행 중...")
            subprocess.Popen([sys.executable, main_controller_path])
            
            # main_controller가 시작될 시간을 주기 위해 잠시 대기
            import time
            time.sleep(2)
            
            # 2. navigation_hud.py 실행
            navigation_hud_path = os.path.join(current_dir, 'navigation_hud.py')
            print(f"   2. navigation_hud.py 실행 중...")
            subprocess.Popen([sys.executable, navigation_hud_path])
            
            # 3. parking_topview.py 실행
            parking_topview_path = os.path.join(current_dir, 'parking_topview.py')
            print(f"   3. parking_topview.py 실행 중...")
            subprocess.Popen([sys.executable, parking_topview_path])
            
            print(f"✅ 모든 프로세스 실행 완료!")
            QApplication.quit()
            
        except FileNotFoundError as e:
            print(f"❌ 실행 실패: 파일을 찾을 수 없습니다. {e}")
            self.show_home() # 오류 발생 시 홈 화면으로 복귀
        except Exception as e:
            print(f"❌ 실행 실패: 실행 중 오류 발생: {e}")
            self.show_home() # 오류 발생 시 홈 화면으로 복귀
