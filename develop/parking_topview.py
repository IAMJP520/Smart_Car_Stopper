...
class CarItem(QGraphicsObject):
    ...
    def boundingRect(self):
        # 원 아이콘 크기에 맞게 조정
        circle_radius = 40
        return QRectF(-circle_radius - 5, -circle_radius - 5, 
                     (circle_radius + 5) * 2, (circle_radius + 5) * 2)

    def paint(self, painter, option, widget):
        painter.setRenderHint(QPainter.Antialiasing)

        # 동그란 원 안에 삼각형이 들어가있는 네비게이션 아이콘
        # 원의 크기
        circle_radius = 40
        ...
        # 외부 원 그리기 (흰색 또는 파란색)
        painter.setBrush(QBrush(QColor(255, 255, 255, 200)))  # 반투명 흰색
        painter.setPen(QPen(QColor(0, 120, 215), 3))  # 파란색 테두리
        painter.drawEllipse(QPointF(0, 0), circle_radius, circle_radius)
        
        # 내부 삼각형 그리기 (위쪽을 향하는 삼각형)
        triangle_size = circle_radius * 0.6  # 원 크기의 60%
        triangle_points = QPolygonF([
            QPointF(0, -triangle_size),  # 위쪽 꼭짓점
            QPointF(-triangle_size * 0.866, triangle_size * 0.5),  # 왼쪽 아래
            QPointF(triangle_size * 0.866, triangle_size * 0.5)  # 오른쪽 아래
        ])
        
        # 삼각형 색상 (파란색)
        painter.setBrush(QBrush(QColor(0, 120, 215)))  # 파란색
...
class ParkingLotUI(QWidget):
    ...
    def setup_styles(self):
        self.setStyleSheet(f"""
            QWidget {{ background: qlineargradient(x1:0, y1:0, x2:1, y2:1, stop:0 #e4dcd3, stop:1 #e4dcd3); color: {HYUNDAI_COLORS['text_primary']}; font-family: 'Malgun Gothic'; }}
            QGraphicsView {{ border: 3px solid {HYUNDAI_COLORS['accent']}; border-radius: 15px; background: '#e4dcd3'; }}
        """)
    ...
    def init_ui(self):
        ...
        self.scene.setBackgroundBrush(QBrush(QColor("#FAF0E6")))  # 따뜻한 아이보리 배경 색상
    ...
    def init_map(self):
        ...
        # 점유된 주차장의 차량 제거 (주황색만 유지)
        # parked_car_positions = {}  # 빈 딕셔너리로 변경
    ...
    def add_block(self, x, y, w, h, color, label=""):
        ...
        if "장애인" in label:
            ...
            gradient = QLinearGradient(x,y,x+w,y+h)
            gradient.setColorAt(0,QColor("#e4dcd3"))
            gradient.setColorAt(1,QColor("#e4dcd3"))
            r.setBrush(QBrush(gradient))
        else:
        ...
        if "장애인" in label or "전기" in label or "일반" in label:
            ...
            pen = QPen(QColor("white"), 20)  # 노란색에서 흰색으로 변경
            r.setPen(pen)
        elif "입출차" in label:
        ...
        if label:
            # 목적지 입구 글자들은 add_block에서 생성하지 않고 build_static_layout에서 특정 좌표에 직접 추가
            if label in ["백화점 본관 입구", "영화관 입구", "문화시설 입구"]:
                pass  # 글자는 build_static_layout에서 특정 좌표에 추가
    ...
    def add_hatched(self, x, y, w, h, edge=QColor("black"), fill=QColor(220, 20, 60, 90)):
        r = QGraphicsRectItem(QRectF(x,y,w,h)); b = QBrush(fill); b.setStyle(Qt.BDiagPattern); r.setBrush(b); r.setPen(QPen(Qt.NoPen)); r.setParentItem(self.layer_static)
        # 통행 불가 글자 제거
    ...
    def build_static_layout(self):
        c_dis, c_ele, c_gen, c_obs, c_emp, c_io = QColor(135, 206, 250), QColor(0, 200, 130), QColor("#e4dcd3"), QColor(108, 117, 125), QColor("#e4dcd3"), QColor("#e4dcd3")
        
        # 전체 배경 박스 추가 (따뜻한 아이보리 색상)
        background_box = QGraphicsRectItem(0, 0, self.SCENE_W, self.SCENE_H)
        gradient = QLinearGradient(0, 0, self.SCENE_W, self.SCENE_H)
        gradient.setColorAt(0, QColor("#FAF0E6"))
        gradient.setColorAt(1, QColor("#FFF8E7"))
        background_box.setBrush(QBrush(gradient))
        background_box.setPen(QPen(Qt.NoPen))
        background_box.setParentItem(self.layer_static)
        background_box.setZValue(-100)  # 가장 뒤에 배치
        
        border = QGraphicsRectItem(0, 0, self.SCENE_W, self.SCENE_H); border.setPen(QPen(QColor("white"), 12)); border.setBrush(QBrush(Qt.NoBrush)); border.setParentItem(self.layer_static)
        ...
        # 일반 주차장 배경 색상 박스 추가
        # (0, 400, 400, 400) 박스
        ivory_box1 = QGraphicsRectItem(QRectF(0, 400, 400, 400))
        gradient1 = QLinearGradient(0, 400, 400, 800)
        gradient1.setColorAt(0, QColor("#e4dcd3"))
        gradient1.setColorAt(1, QColor("#e4dcd3"))
        ivory_box1.setBrush(QBrush(gradient1))
        ivory_box1.setPen(QPen(Qt.NoPen))
        ivory_box1.setParentItem(self.layer_static)
        
        # (0, 800, 1600, 800) 박스
        ivory_box2 = QGraphicsRectItem(QRectF(0, 800, 1600, 800))
        gradient2 = QLinearGradient(0, 800, 1600, 1600)
        gradient2.setColorAt(0, QColor("#e4dcd3"))
        gradient2.setColorAt(1, QColor("#e4dcd3"))
        ivory_box2.setBrush(QBrush(gradient2))
        ivory_box2.setPen(QPen(Qt.NoPen))
        ivory_box2.setParentItem(self.layer_static)
        ...
        # 목적지 입구 글자를 특정 좌표에 추가
        font_large = QFont("Malgun Gothic", int(FONT_SIZES['map_label'] * 2.25), QFont.Bold)
        
        # (-200, 1800)에 "백화점 본관 입구" (두 줄로 표시)
        t1 = QGraphicsSimpleTextItem("백화점")
        t1.setFlag(QGraphicsItem.ItemIgnoresTransformations, True)
        t1.setBrush(QColor(0, 0, 0))  # 검정색
        t1.setFont(font_large)
        t1.setPos(-270, 1880)
        t1.setParentItem(self.layer_static)
        
        t2 = QGraphicsSimpleTextItem("본관 입구")
        t2.setFlag(QGraphicsItem.ItemIgnoresTransformations, True)
        t2.setBrush(QColor(0, 0, 0))  # 검정색
        t2.setFont(font_large)
        t2.setPos(-300, 1800)  # "백화점" 아래에 배치
        t2.setParentItem(self.layer_static)
        
        # (1800, 1800)에 "영화관 입구"
        t3 = QGraphicsSimpleTextItem("영화관 입구")
        t3.setFlag(QGraphicsItem.ItemIgnoresTransformations, True)
        t3.setBrush(QColor(0, 0, 0))  # 검정색
        t3.setFont(font_large)
        t3.setPos(1675, 1840)
        t3.setParentItem(self.layer_static)
        
        # (1800, 600)에 "문화시설 입구"
        t4 = QGraphicsSimpleTextItem("문화시설 입구")
        t4.setFlag(QGraphicsItem.ItemIgnoresTransformations, True)
        t4.setBrush(QColor(0, 0, 0))  # 검정색
        t4.setFont(font_large)
        t4.setPos(1650, 640)
        t4.setParentItem(self.layer_static)
...
if __name__ == "__main__":
    ...
    app.setStyleSheet(f"""
        QApplication {{ background-color: '#e4dcd3'; }}
    """)
