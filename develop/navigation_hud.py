...
class PremiumHudWidget(QFrame):
    ...
    def paintEvent(self, event):
        ...
        # 창 크기에 맞춰 동적으로 배치 (간격 충분히 확보하여 겹치지 않게)
        self.draw_background_effects(painter, rect)
        
        # 방향 표시: 상단에서 시작 (반지름 170, 실제 높이 340px)
        # 최소 높이에서도 안전하게 배치
        direction_y = max(220, int(height * 0.12 + 170))
        self.draw_3d_direction_display(painter, center_x, direction_y)
        
        # 거리 패널: 방향 표시 하단(170) + 여유공간(50) + 패널 상단(100) = +320
        # 패널은 y-100 ~ y+100 이므로 방향 표시 y+170 + 50 = y+220부터 시작
        distance_y = int(direction_y + 320)  # 170(반지름) + 50(여유) + 100(패널 상단)
        self.draw_distance_panel(painter, center_x, distance_y)
        
        # 속도 게이지: 거리 패널 하단(100) + 여유공간(40) + 게이지 상단(80) = +220
        # 게이지는 y-80 ~ y+80이므로 거리 패널 y+100 + 40 = y+140부터 시작
        speed_y = int(distance_y + 220)  # 100(패널 하단) + 40(여유) + 80(게이지 상단)
        self.draw_speed_gauge(painter, center_x, speed_y)
        
        # 다음 안내 카드: 속도 게이지 하단(80) + 여유공간(40) + 카드 상단(80) = +200
        # 카드는 y-80 ~ y+80이므로 게이지 y+80 + 40 = y+120부터 시작
        next_y = int(speed_y + 200)  # 80(게이지 하단) + 40(여유) + 80(카드 상단)
        self.draw_next_instruction_card(painter, center_x, next_y)
    ...
    def draw_3d_direction_display(self, painter, center_x, y):
        ...
        # 배경 원형 모두 제거 - 화살표만 표시
        # pulse_scale과 2배 확대를 함께 적용
        painter.scale(self.pulse_scale * 2.0, self.pulse_scale * 2.0)
    ...
    def draw_3d_destination_icon(self, painter, x, y, shadow=False):
        # scale이 2배로 적용되어 있으므로 원래 크기 사용
        if shadow:
            painter.drawEllipse(QPointF(x, y), 50, 50)
        else:
            painter.setBrush(QBrush(QColor(HYUNDAI_COLORS['blue_soft'])))  # 파란색으로 변경
            painter.setPen(QPen(QColor(HYUNDAI_COLORS['blue_soft']), 4))  # scale 적용되므로 원래 두께
            painter.drawEllipse(QPointF(x, y), 50, 50)
            painter.setBrush(QBrush(QColor(HYUNDAI_COLORS['blue_soft'])))
            painter.setPen(Qt.NoPen)
    ...
    def draw_3d_exit_complete_icon(self, painter, x, y, shadow=False):
        ...
        if shadow:
            ...
            painter.setPen(QPen(QColor(HYUNDAI_COLORS['blue_soft']), 4))  # scale 적용되므로 원래 두께
            painter.drawEllipse(QPointF(x, y), 50, 50)
            painter.setPen(QPen(QColor(HYUNDAI_COLORS['blue_soft']), 6))  # scale 적용되므로 원래 두께
    ...
    def draw_distance_panel(self, painter, center_x, y):
        ...
        painter.drawText(QRectF(center_x-300, y-60, 600, 120), Qt.AlignCenter, distance_text)
        ...
        painter.drawText(QRectF(center_x-300, y+20, 600, 80), Qt.AlignCenter, direction_text)
        painter.restore()
    ...
    def draw_next_instruction_card(self, painter, center_x, y):
        ...
        painter.drawText(QPointF(center_x-380, y-30), "다음")
