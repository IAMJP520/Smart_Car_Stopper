...
class DummyCarClient:
    ...
    def send_json(self, data):
        ...
        try:
            ...
            # main_controller.py는 완전한 JSON 문자열을 기대하며, 
            # 여러 메시지가 연속으로 올 수 있으므로 줄바꿈 없이 전송
            self.client_socket.sendall(message.encode('utf-8'))
            # 전송 후 약간의 대기 시간 (서버 처리 시간 확보)
            time.sleep(0.01)
        except Exception as e:
            print(f"❌ 데이터 전송 오류: {e}")
    ...
    # ====== 기능: 경로 전송 ======
    def send_waypoints(self, waypoints, parking_spot=None, route_type='entry'):
        """지정된 웨이포인트 경로를 서버에 전송합니다.
        
        Args:
            waypoints: 웨이포인트 리스트 [[x1, y1], [x2, y2], ...]
            parking_spot: 주차구역 번호 (선택적)
            route_type: 'entry' (입차) 또는 'exit' (출차)
        """
        print("\n" + "="*50)
        print(f"🗺️  웨이포인트 경로를 전송합니다: {waypoints}")
        # main_controller.py가 기대하는 형식으로 변경
        waypoint_data = {
            "type": "waypoint",
            "waypoints": waypoints,
    ...
    def control_with_keyboard(self, start_pos, step=25, send_interval=0.02, tag_id="dummy_car_01"):
        ...
        def send_pos(px, py):
            ...
            data = {
                ...
                "x": px,
                "y": py,
                "heading": 0,  # 방향각 (선택적, 기본값 0)
                "speed": step * 10  # 속도 (선택적, step에 비례)
            }
            self.send_json(data)
        ...
        for i in range(len(waypoints) - 1):
            ...
            for step in range(num_steps + 1):
                ...
                position_data = {
                    ...
                    "x": current_pos_x,
                    "y": current_pos_y,
                    "heading": math.atan2(dy, dx) * 180 / math.pi if dx != 0 or dy != 0 else 0,
                    "speed": speed  # 초당 픽셀 단위 속도
                }
    ...
    def generate_route_to_parking(self, parking_spot_num, parking_coords):
        ...
        if parking_spot_num == 1:  # 1번: 입구 -> (200,1475)
            ...
            waypoints.append(target_waypoint)
        elif parking_spot_num == 7:  # 7번: 입구 -> (200, 925) -> (1475, 925) -> (1475, 1000)
            waypoints.append([200, 925])
            waypoints.append([1475, 925])
            waypoints.append(target_waypoint)
        elif parking_spot_num in [8, 9, 10, 11]:  # 8~11번: 입구 -> (200, 925) -> 최종 주차구역
            waypoints.append([200, 925])
            waypoints.append(target_waypoint)
    ...
    def generate_reroute_waypoints(self, current_position, target_parking_spot):
        ...
        if target_parking_spot == 1:  # 1번: 현재위치 -> (200,1475)
            if current_position[1] < 1475:  # 아직 위로 올라가야 함
            ...
            if current_position[0] < target_waypoint[0]:  # 아직 오른쪽으로 가야 함
                new_waypoints.append(target_waypoint)
        elif target_parking_spot == 6:  # 6번: 현재위치 -> (200, 1475) -> (1475, 1475) -> (1475, 1400)
            if current_position[1] < 1475:
                new_waypoints.append([200, 1475])
            if current_position[0] < 1475:
                new_waypoints.append([1475, 1475])
            if current_position[1] > 1400:
                new_waypoints.append([1475, 1400])
        elif target_parking_spot == 7:  # 7번: 현재위치 -> (200, 925) -> (1475, 925) -> (1475, 1000)
            if current_position[1] > 925:  # 아직 아래로 내려가야 함
                new_waypoints.append([200, 925])
            if current_position[0] < 1475:  # 아직 오른쪽으로 가야 함
                new_waypoints.append([1475, 925])
            if current_position[1] > 1000:  # 아직 아래로 내려가야 함
                new_waypoints.append([1475, 1000])
        elif target_parking_spot in [8, 9, 10, 11]:  # 8~11번: 현재위치 -> (200, 925) -> 최종 주차구역
            if current_position[1] > 925:  # 아직 아래로 내려가야 함
                new_waypoints.append([200, 925])
            if current_position[0] < target_waypoint[0]:  # 아직 오른쪽으로 가야 함
                new_waypoints.append(target_waypoint)
    ...
    def send_reroute_request(self, current_position, target_parking_spot):
        """경로 재탐색 요청을 서버에 전송합니다."""
        print(f"\n🔄 경로 재탐색 요청 전송")
        print(f"   현재 위치: {current_position}")
        print(f"   목적지: {target_parking_spot}번 주차구역")
        
        new_waypoints = self.generate_reroute_waypoints(current_position, target_parking_spot)
        
        if new_waypoints:
            print(f"   새로운 경로: {new_waypoints}")
            # 주차구역 정보와 함께 전송
            self.send_waypoints(new_waypoints, parking_spot=target_parking_spot, route_type='entry')
            return True
        else:
            print("❌ 경로 재탐색 실패")
            return False
    ...
    def run_scenario(self, manual=True):
        ...
        # 주차구역 정보와 함께 전송
        self.send_waypoints(waypoints_to_send, parking_spot=parking_spot_num, route_type='entry')
