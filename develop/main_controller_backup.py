...
class ExternalServerReceiver:
    ...
    def process_received_data(self, json_str: str, client_socket=None):
        ...
        try:
            ...
            if data_type == 'position' and self.broadcaster:
                ...
                # 외부 정산 서버로 정산 확인 전달
                print(f"➡ 외부 정산 서버로 확인 전달 준비: {self.receiver.payment_server_host}:{self.receiver.payment_server_port}")
    ...
    def send_payment_confirmation_to_external_server(self, confirmed: bool, amount: int, parking_spot: int) -> None:
        ...
        try:
            print(f"🔌 외부 정산 서버 연결 시도: {self.receiver.payment_server_host}:{self.receiver.payment_server_port}")
            
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            sock.settimeout(5.0)
            try:
                sock.connect((self.receiver.payment_server_host, self.receiver.payment_server_port))
                ...
                print(f"❌ 정산 확인 전송 타임아웃: {self.receiver.payment_server_host}:{self.receiver.payment_server_port}")
            except ConnectionRefusedError:
                print(f"❌ 정산 확인 전송 실패(연결 거부): {self.receiver.payment_server_host}:{self.receiver.payment_server_port}")
            finally:
                sock.close()
        except Exception as e:
