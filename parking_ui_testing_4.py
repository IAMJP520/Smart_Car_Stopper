...
# 다른 목적지 예시들:
# DESTINATIONS = {
#     'main': (800, 1800),       # 장애인 주차구역
#     'waypoint1': None,         # 경유지 없음
#     'waypoint2': None,
# }

# DESTINATIONS = {
#     'main': (1300, 1800),      # 전기차 주차구역
#     'waypoint1': (1000, 1200), # 중간 경유지
#     'waypoint2': None,
# }
...
class ParkingLotUI(QWidget):
    ...
    def __init__(self):
        ...
        # 목적지 정보 표시
        dest_info = QLabel(f"🎯 설정된 목적지:")
        dest_info.setStyleSheet(f"""
            QLabel {{
        ...
        main_dest.setStyleSheet(f"color: {HYUNDAI_COLORS['text_primary']}; font-size: 14px;")
        ...
        waypoint1_dest.setStyleSheet(f"color: {HYUNDAI_COLORS['text_secondary']}; font-size: 12px;")
        ...
        waypoint2_dest.setStyleSheet(f"color: {HYUNDAI_COLORS['text_secondary']}; font-size: 12px;")
    ...
    def start_navigation(self):
        """코드에서 설정된 목적지로 네비게이션 시작"""
        # DESTINATIONS 딕셔너리에서 좌표 가져오기
        waypoints = []
        
        # 주 목적지는 필수
        if DESTINATIONS['main']:
            main_point = QPointF(DESTINATIONS['main'][0], DESTINATIONS['main'][1])
            waypoints.append(self.clamp_point(main_point))
        
        # 경유지들은 선택사항
        if DESTINATIONS['waypoint1']:
            wp1_point = QPointF(DESTINATIONS['waypoint1'][0], DESTINATIONS['waypoint1'][1])
            waypoints.insert(0, self.clamp_point(wp1_point))  # 경유지1을 먼저 방문
            
        if DESTINATIONS['waypoint2']:
            wp2_point = QPointF(DESTINATIONS['waypoint2'][0], DESTINATIONS['waypoint2'][1])
            if len(waypoints) == 1:  # 경유지1이 없는 경우
                waypoints.insert(0, self.clamp_point(wp2_point))
            else:  # 경유지1이 있는 경우 그 다음에 방문
                waypoints.insert(1, self.clamp_point(wp2_point))
        
        if not waypoints:
            QMessageBox.warning(self, "설정 오류", "목적지가 설정되지 않았습니다.")
            return
