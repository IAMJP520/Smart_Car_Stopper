...
class PremiumHudWidget(QFrame):
    ...
    def update_navigation_info(self, instructions, current_speed=0, route_progress=0):
        ...
        if is_turn_complete and len(instructions) > 1:
            ...
            if "목적지" in next_dir and next_dist > 8:
...
class ParkingLotUI(QWidget):
    ...
    def __init__(self):
        ...
        self.deviation_threshold = 3.0  # 3초
    ...
    def update_ui_with_waypoints(self, waypoints):
        ...
        if hasattr(self.hud, 'voice_guide'):
            # 첫 번째 음성 재생 후 두 번째 음성을 재생하도록 콜백 설정
            self.hud.voice_guide.speak_instruction("경로 안내를 시작합니다")
            
            # 경로 계산 완료 후 첫 번째 안내 음성을 재생하도록 타이머 설정
            QTimer.singleShot(3000, self.play_first_navigation_instruction)  # 3초 후 실행
        
        QMessageBox.information(self, "경로 자동 설정", f"새로운 경로가 수신되었습니다:\n{waypoints}\n\n자동으로 경로 안내를 시작합니다.")
        self.calculate_and_display_route()

    def play_first_navigation_instruction(self):
        """첫 번째 네비게이션 안내 음성 재생"""
        if hasattr(self.hud, 'voice_guide') and self.full_path_points and len(self.full_path_points) >= 2:
            # 첫 번째 경로 세그먼트의 방향과 거리 계산
            start_point = self.full_path_points[0]
            end_point = self.full_path_points[1]
            
            # 거리 계산 (픽셀을 미터로 변환)
            distance_m = sqrt((end_point.x() - start_point.x())**2 + (end_point.y() - start_point.y())**2) / 50
            ...
            # 음성 메시지 생성 및 재생
            if distance_m > 8:
                voice_message = f"{int(distance_m)}미터 직진입니다"
            else:
                voice_message = f"{int(distance_m)}미터 직진입니다"
            
            print(f"🔊 첫 번째 네비게이션 안내: {voice_message}")
            self.hud.voice_guide.speak_instruction(voice_message)
    ...
    def show_route_recalculation_popup(self):
        ...
        # TTS 음성 안내 추가
        if hasattr(self.hud, 'voice_guide'):
            self.hud.voice_guide.speak_instruction("경로를 재탐색합니다. 잠시만 기다려주세요.")
